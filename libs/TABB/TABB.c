...
int mNAux(TABB* tabb, int N, int* vetor, int pos){
    if(tabb == NULL) return pos;

    if(tabb->info < N){
      vetor[pos] = tabb->info;
      pos++;
      pos = mNAux(tabb->dir, N, vetor, pos);
    }
    
    pos = mNAux(tabb->esq, N, vetor, pos);

    return pos;
  }

int* mN(TABB*tabb, int N, int *tam){
    int* vetor = (int*)malloc(sizeof(int)*(*tam));

    *tam = mNAux(tabb, N, vetor, 0);
